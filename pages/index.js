...
export default function Home() {
  ...
  const createDocument = () => {
    ...
    db.collection('useDocs').doc(session.user.email).collection('docs').add({
      fileName: input,
  ...
  const modal = (
    <Modal
        size='sm'
        active={showModal}
        toggle={() => setShowModal(false) || true}
    >
      <ModalBody>
        <input value={input} onChange={e => setInput(e.target.value)} type='text' className='outline-none w-full' placeholder='Enter name of document...' onKeyDown={e => e.key === 'Enter' && createDocument()} />
      </ModalBody>
      <ModalFooter>
        <Button
        color='blue'
      ...
      <section className='bg-white px-10 md:px-0'>
        <div className="max-w-3xl mx-auto py-8 text-sm text-gray-700">
          <div className="flex items-center justify-between pb-5">
            ...
            <p className='mr-12'>Date Create</p>
